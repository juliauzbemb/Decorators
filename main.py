from decorator import logger, logger_params
...
class Student:
    ...
    def lecturer_rate_hw(self, lecturer, course, grade):
        if isinstance(lecturer, Lecturer) and course \
                in lecturer.courses_attached and \
    ...
    def __str__(self):
        res1 = f'Имя: {self.name}\nФамилия:' \
               f' {self.surname}\nСредняя ' \
               f'оценка за домашние задания: ' \
               f'{self.get_avg_grade_st()}\nКурсы в процессе изучения: ' \
...
@logger_params('log_additional.txt')
